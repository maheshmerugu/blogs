...
(function (mod) {
    ...
    function updateGutterSpace(cm) {
        ...
        cm.display.scrollbarH.style.left = cm.options.fixedGutter
            ? width + "px"
            : 0;
    }
    ...
    function hScrollbarTakesSpace(cm) {
        return (
            cm.display.scroller.clientHeight - cm.display.wrapper.clientHeight <
            scrollerCutOff - 3
        );
    }
    ...
    function updateScrollbars(cm, measure) {
            ...
            d.scrollbarV.firstChild.style.height =
                Math.max(
                    ...
                    scrollHeight -
                        ...
                        (measure.barHeight || d.scrollbarV.clientHeight)
                ) + "px";
        } else {
        ...
        if (needsH) {
            ...
            d.scrollbarH.firstChild.style.width =
                measure.scrollWidth -
                measure.clientWidth +
                (measure.barWidth || d.scrollbarH.clientWidth) +
                "px";
        } else {
        ...
        if (!cm.state.checkedOverlayScrollbar && measure.clientHeight > 0) {
            if (sWidth === 0) {
                ...
                var barMouseDown = function (e) {
                    if (
                        e_target(e) != d.scrollbarV &&
                        e_target(e) != d.scrollbarH
                    )
    ...
    function postUpdateDisplay(cm, update) {
        ...
        if (
            cm.display.viewFrom != update.oldViewFrom ||
            cm.display.viewTo != update.oldViewTo
        )
            signalLater(
                cm,
                "viewportChange",
    ...
    function setDocumentHeight(cm, measure) {
        ...
        cm.display.gutters.style.height =
            Math.max(measure.docHeight, measure.clientHeight - scrollerCutOff) +
            "px";
    }
    ...
    function checkForWebkitWidthBug(cm, measure) {
        ...
        if (
            cm.display.sizer.offsetWidth + cm.display.gutters.offsetWidth <
            cm.display.scroller.clientWidth - 1
        ) {
            cm.display.sizer.style.minHeight =
                cm.display.heightForcer.style.top = "0px";
    ...
    function updateLineGutter(cm, lineView, lineN, dims) {
        ...
        if (cm.options.lineNumbers || markers) {
            var wrap = ensureLineWrapped(lineView);
            var gutterWrap = (lineView.gutter = wrap.insertBefore(
                elt(
                    "div",
                    null,
    ...
    function measureCharInner(cm, prepared, ch, bias) {
        ...
        if (node.nodeType == 3) {
            ...
            for (var i = 0; i < 4; i++) {
                ...
                while (
                    start &&
                    isExtendingChar(prepared.line.text.charAt(mStart + start))
                )
                    --start;
                while (
                    mStart + end < mEnd &&
    ...
    function endOperation_finish(op) {
        ...
        if (
            ...
            Math.abs(
                op.barMeasure.scrollWidth - cm.display.scroller.scrollWidth
            ) > 1
        )
            updateScrollbars(cm);
        ...
        if (op.updatedDisplay && webkit) {
            ...
            if (
                op.barMeasure.scrollWidth > op.barMeasure.clientWidth &&
    ...
    function readInput(cm) {
        ...
        if (cm.state.pasteIncoming && doc.sel.ranges.length > 1) {
            if (lastCopied && lastCopied.join("\n") == inserted)
                multiPaste =
                    doc.sel.ranges.length % lastCopied.length == 0 &&
    ...
    function onMouseDown(e) {
        ...
        switch (e_button(e)) {
            case 1:
                if (start) leftButtonDown(cm, e, start);
    ...
    function leftButtonSelect(cm, e, start, type, addNew) {
        ...
        if (addNew && !e.shiftKey) {
            ...
            if (ourIndex > -1) ourRange = doc.sel.ranges[ourIndex];
        ...
        if (!addNew) {
            ...
            setSelection(
                doc,
                normalizeSelection(doc.sel.ranges.concat([ourRange]), ourIndex),
    ...
    function onKeyPress(e) {
        ...
        if (
            ((presto && (!e.which || e.which < 10)) || khtml) &&
    ...
    function onContextMenu(cm, e) {
        ...
        function rehide() {
            ...
            if (ie && ie_version < 9)
                display.scrollbarV.scrollTop = display.scroller.scrollTop =
                    scrollPos;
            slowPoll(cm);
        ...
        if (doScroll != null && !phantom) {
            var scrollNode = elt(
                ...
                "position: absolute; top: " +
                    ...
                    "px; height: " +
                    (coords.bottom - coords.top + scrollerCutOff) +
                    "px; left: " +
                    coords.left +
                    "px; width: 2px;"
            );
    ...
    function widgetHeight(widget) {
        ...
        if (!contains(document.body, widget.node)) {
            ...
            if (widget.coverGutter)
                parentStyle +=
                    "margin-left: -" +
                    widget.cm.getGutterElement().offsetWidth +
                    "px;";
        ...
        addLineClass: docMethodOp(function (handle, where, cls) {
            return changeLine(this, handle, "class", function (line) {
                ...
                if (!line[prop]) line[prop] = cls;
                    ...
                    new RegExp("(?:^|\\s)" + cls + "(?:$|\\s)").test(line[prop])
                )
                    return false;
                else line[prop] += " " + cls;
                return true;
